...
export function evaluateCustomerAcknowledgement(
  ...
  // Extract explicit client turns even if single-line or multiple lines
  const clientTurns: string[] = [];
  const turnRegex = /(?:^|[\n\r]|(?<=[.?!;]\s*))\s*(?:client|customer|caller|speaker\s*2)\s*:\s*([^]+?)(?=(?:[\n\r]|(?<=[.?!;]\s*))\s*(?:advisor|agent|dealer|speaker\s*1)\s*:|$)/gi;
  ...
  if (clientTurns.length > 0) {
    ...
    } else if (/(?:advisor|agent|dealer|speaker\s*1)\s*:/i.test(transcript)) {
